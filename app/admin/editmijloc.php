...
try {
  ...
  $mijloc = $dbh -> query("SELECT * FROM mijloace WHERE ID = '$id_mijloc'");
  foreach($mijloc as $row)
  {
    ...
    <div>
        <p>Cantitate</p>
        <input type="number" name="cantitate" value="<?php echo $row['cantitate']; ?>" required />
    </div>
    <div>
        <p>Nr. Inventar</p>
        <input type="number" name="nr_inventar" value="<?php echo $row['numar_inventar']; ?>" required />
    </div>
    <div style="text-align: right;">
        <input type="hidden" name="id_mijloc" value="<?php echo $id_mijloc; ?>" required />
